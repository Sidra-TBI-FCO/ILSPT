...
load(paste0("./Analysis/after_split/Signature_Enrichment/GSEA_PanCancer_pediatric_only_Bindea_REV1.Rdata"))
...
#write.csv(HR_table,file = paste0("./Analysis/after_split/Survival_Analysis/Figure.1G.HR_table_",Cancer,"_ES_",Gene.set,"_cutoff_", Surv.cutoff.years,".csv"))
...
load("./Analysis/after_split/Signature_Enrichment/order_signatures/032.Selected_only.sig.p.value.Rdata")
...
pdf(file = paste0("./Figures/after_split/Forest_plots/",Gene.set,"_forest.plots/032.",Gene.set,"_",Cancer,"_Forest_plot_ES",Gene.set,"_cutoff_", Surv.cutoff.years, ".pdf"),
    height = 7, width = 7)
dev.new()
...
plot_df$Cancer = annotation_all$Cancer[match(rownames(plot_df),
                                             annotation_all$Sample)]


plot_df$Cancer = factor(plot_df$Cancer, levels = c("CCSK","OS","RT","WT", "NBL_mycn_Amp","NBL_mycn_Namp_Intermed.Low","NBL_mycn_Namp_High"))
#plot_df$cluster = factor(plot_df$cluster, levels = c("S1","S2", "S3", "S4" , "S5" , "S6"))

plot_df_agg = aggregate(.~Cancer, plot_df, FUN = median)


rownames(plot_df_agg) = plot_df_agg$Cancer
plot_df_agg$Cancer = NULL
...
#load("./Analysis/after_split/Signature_Enrichment/order_signatures/Selected_only.sig.p.value.Rdata")

load(paste0("./Analysis/after_split/Signature_Enrichment/",Gene.set,"_genes_order.Rdata"))
plot_df_agg_z = plot_df_agg_z[which(rownames(plot_df_agg_z) %in% genes_order),]

plot_df_agg_z = plot_df_agg_z[genes_order,]

#write.csv(plot_df_agg_z,file = paste0("./Analysis/after_split/Signature_Enrichment/Figure.1H.zscored.matrix.ES.allCancers.csv"))

ha = HeatmapAnnotation(`Cancer` = colnames(plot_df_agg_z),
...
png(paste0("./Figures/after_split/Dotted_Heatmap/032.",Gene.set,"_CancerTypes_pediatric_only.png"),
res = 600, width = 11, height = 13, units = "in")

Heatmap(plot_df_agg_z, cluster_rows = FALSE, cluster_columns = FALSE,
        show_column_names = FALSE, top_annotation = ha,name = "Mean expression (z-scored)",
...
load(paste0("./Analysis/after_split/Signature_Enrichment/GSEA_PanCancer_pediatric_only_Bindea_REV1.Rdata"))
ES = ES[,which(colnames(ES) %in% clinical_data$submitter_id)]
load("./Analysis/after_split/annotation_with_NBL_subgroups.Rdata")
annotation_all = annotation_all[which(annotation_all$Sample %in% clinical_data$submitter_id),]
...
#load("./Analysis/after_split/Signature_Enrichment/Selected.pathways_genes_order.Rdata")
#HR_table = HR_table[genes_order,]
rownames(HR_table) = NULL

dir.create(paste0("./Analysis/after_split/Survival_Analysis/HR_for_Heat/",Gene.set), showWarnings = FALSE)
save(HR_table, file = paste0("./Analysis/after_split/Survival_Analysis/HR_for_Heat/",Gene.set,"/HR_table_",Cancer,"_ES_",Gene.set,"_cutoff_", Surv.cutoff.years,"pediatric.Rdata"))
...
df = data.frame(matrix(nrow=26, ncol =8 ))
...
for (i in 1:N.sets){
  ...
  load(paste0("./Analysis/after_split/Survival_Analysis/HR_for_Heat/",Gene.set,"/HR_table_",Cancer,"_ES_",Gene.set,"_cutoff_", Surv.cutoff.years,"pediatric.Rdata"))
  HR_table$Signature = as.character(HR_table$Signature)
  rownames(HR_table) = HR_table$Signature
  HR_table = HR_table[genes_order,]
  df[,Cancer] = HR_table$HR[match(rownames(df),HR_table$Signature)]
  df = df[genes_order,]
}

df_HR = data.frame(matrix(nrow=26, ncol = 8))
colnames(df_HR) = CancerTYPES
...
for (i in 1:N.sets){
  ...
  rownames(HR_table) = HR_table$Signature
  HR_table = HR_table[genes_order,]
  df_HR[,Cancer] = HR_table$HR[match(rownames(df_HR),HR_table$Signature)]
  df_pvalue[,Cancer] = HR_table$p_value[match(rownames(df_pvalue),HR_table$Signature)]
}
...
write.csv(df_pvalue,file = paste0("./Analysis/after_split/Survival_Analysis/Figure.1I.pvalue.HR.matrix.allCancers.csv"))
write.csv(df_HR,file = paste0("./Analysis/after_split/Survival_Analysis/Figure.1I.HR.HR.matrix.allCancers.csv"))
...
dir.create(paste0("./Figures/after_split/Heatmaps/HR_heatmap/",Gene.set,"/"),showWarnings = FALSE)
png(filename = paste0("./Figures/after_split/Heatmaps/HR_heatmap/",Gene.set,"/023.Complexheatmap_", Gene.set, "_Pancancer_plus_NBL_groups.png"), res = 600,
    width = 11, height = 13, units = "in")
...
HM_HR = Heatmap(df_HR, cluster_rows = FALSE ,cluster_columns = FALSE , row_names_max_width = unit(6, "in"),column_title_gp = gpar(fontsize = 14),row_names_gp = gpar(fontsize = 22),
                # heatmap_legend_param =list(title_gp=gpar(fontsize=20),legend_width=unit(10,"cm")),
                show_column_names = FALSE, top_annotation = ha, name = "HR",column_order = c("Pancancer","CCSK","OS","RT","WT", "NBL_mycn_Amp","NBL_mycn_Namp_Intermed.Low","NBL_mycn_Namp_High"),
                rect_gp = gpar(type = "none"),
                cell_fun = function(j, i, x, y, width, height, fill) {
                  grid.rect(x = x, y = y, width = width, height = height, gp = gpar(col = "white", fill = col_fun2(df_pvalue_categories[i,j])))
                  grid.circle(x = x, y = y, r = abs(df_pvalue[i, j])/3 * min(unit.c(width, height)) ,
...
order_signatures = rownames(df_HR)[rcl.list]

dir.create("./Analysis/after_split/Signature_Enrichment/order_signatures",showWarnings = FALSE)
save(order_signatures,file = "./Analysis/after_split/Signature_Enrichment/order_signatures/032.Selected_only.sig.p.value.Rdata")
