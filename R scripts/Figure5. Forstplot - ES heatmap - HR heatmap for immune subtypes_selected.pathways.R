# Four scripts for generating figures 5A,B,C
...
rownames(ES) = gsub(".*] ", "",rownames(ES))
rownames(ES) = gsub("SHC1/pSTAT3 Signature", "SHC1 pSTAT3 Signature",rownames(ES))
rownames(ES) = gsub("Hypoxia/Adenosine Immune Cell Suppression", "Hypoxia Adenosine Immune Cell Suppression",rownames(ES))
rownames(ES) = gsub("[()]","",rownames(ES))
...
#write.csv(plot_df_agg_z,file = paste0("./Analysis/after_split/Signature_Enrichment/Figure.5B.zscored.matrix.ES.allCancers.csv"))

#load(paste0("./Analysis/after_split/Signature_Enrichment/order_signatures/",Gene.set,".sig.p.value.Rdata"))
#plot_df_agg_z = plot_df_agg_z[which(rownames(plot_df_agg_z) %in% order_signatures),]
...
png(paste0("./Figures/after_split/Dotted_Heatmap/", Gene.set, "_immuneSubtypes_pediatric_only.png"),
    res = 600, width = 8, height = 10, units = "in")
...
clinical_data = read.csv(paste0("./Data/Clinical_Data/TARGET-AllCancers_clinical.csv"))
...
annotation_all = annotation_all[which(annotation_all$cluster %in% cluster),]
rownames(clinical_data) = clinical_data$submitter_id
ES = ES[,which(colnames(ES)  %in% annotation_all$Sample)]
...
df = data.frame(matrix(nrow=54, ncol =7 ))
...
df_HR = data.frame(matrix(nrow=54, ncol = 7))
...
#write.csv(df_pvalue,file = paste0("./Analysis/after_split/Survival_Analysis/Figure.5B.pvalue.HR.matrix.allCancers.csv"))
#write.csv(df_HR,file = paste0("./Analysis/after_split/Survival_Analysis/Figure.5C.HR.HR.matrix.allCancers.csv"))
...
png(filename = paste0("./Figures/after_split/Heatmaps/HR_heatmap/ImmuneSubtypes/",Gene.set,"/","Complexheatmap_", Gene.set, "_PLUS_Pancancer_HR_dotted.test.png"), res = 600,
    width = 5, height = 16, units = "in")
...
draw(HM_HR, heatmap_legend_side = NULL, annotation_legend_side = NULL)
dev.off()
...
save(order_signatures,file = paste0("./Analysis/after_split/Signature_Enrichment/order_signatures/",Gene.set,".sig.p.value.Rdata"))
order_signatures
